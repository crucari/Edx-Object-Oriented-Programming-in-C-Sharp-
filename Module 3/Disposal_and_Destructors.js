...
// You can add a destructor to a class to perform any additional application-specific cleanup that is necessary when your class is garbage collected.
// To define a destructor, you add a tilde (~) followed by the name of the class. You then enclose the destructor logic in braces.
...
// When you declare a destructor, the compiler automatically converts it to an override of the Finalize method of the object class.
// However, you cannot explicitly override the Finalize method; you must declare a destructor and let the compiler perform the conversion.

// If you want to guarantee that the Dispose method is always invoked, you can include it as part of the finalization process that the GC performs.
// To do this, you can add a call to the Dispose method in the destructor of the class.
...
// Using types that implement the IDisposable interface is not sufficient to manage resources.
// You must also remember to invoke the Dispose method in your code when you have finished with the object.
// If you choose not to implement a destructor that invokes the Dispose method when the GC processes the object, you can do this in a number of other ways.
...
// Invoking the Dispose method explicitly after code that uses the object is perfectly acceptable,
// but if your code throws an exception before the call to the Dispose method, the Dispose method will never be invoked.
...
// Any code in the scope of the finally block will always execute, regardless of any exceptions that might be thrown.
// Therefore, with this approach, you can always guarantee that your code will invoke the Dispose method.
...
// Note: When explicitly invoking the Dispose method, it is good practice to check whether the object is not null beforehand,
// because you cannot guarantee the state of the object.

// Alternatively, you can use a using statement to implicitly invoke the Dispose method.
